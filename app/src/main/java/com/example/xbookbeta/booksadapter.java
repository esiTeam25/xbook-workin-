...
public class booksadapter extends RecyclerView.Adapter<booksadapter.bookholder> {
    ...
    public void onBindViewHolder(@NonNull bookholder holder, int position) {
        ...
        DatabaseReference account = FirebaseDatabase.getInstance().getReference().child("users").child(u.getUserid());
    ...
    holder.distance.setText(
            String.format("%.2f",
                    SphericalUtil.computeDistanceBetween(
                            new LatLng(u.getLatitude(), u.getLongitude()),
                            FirstActivity.locationToUpload) / 1000) + "km");
}
        /*
